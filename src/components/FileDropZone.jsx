  ...
  return (
    <div className={`space-y-1.5 ${className}`}>
      <div
        ...
        className={`relative border-2 border-dashed rounded-glass-lg transition-all duration-200 cursor-pointer flex flex-col items-center justify-center text-center select-none ${
          compact ? 'p-3 py-4' : 'p-6'
        } ${
          isDragOver
            ? 'border-brand-500 bg-brand-500/10 scale-[1.01]'
            : 'border-zinc-700/50 hover:border-brand-500/50 bg-zinc-800/40 hover:bg-zinc-800/70'
        }`}
      >
        <input
          ref={fileInputRef}
        ...
        {recentSuccess ? (
          <div className="flex items-center space-x-2 text-green-400 py-1 animate-fadeIn">
          ...
          <div className="space-y-1.5 flex flex-col items-center">
            <div
              className={`rounded-full flex items-center justify-center transition-all ${
                ...
                isDragOver
                  ...
                  : 'bg-zinc-800/50 border border-zinc-700/50 text-brand-400'
              }`}
            >
            ...
            <div>
              <p className={`font-semibold text-white ${compact ? 'text-[11.5px]' : 'text-xs'}`}>
                {label}
              </p>
              {sublabel && (
                <p className={`text-text-subtle mt-0.5 ${compact ? 'text-[10px]' : 'text-[11px]'}`}>
                  {sublabel}
                </p>
              )}
            </div>
      ...
      {error && (
        <div className="flex items-center gap-1.5 text-xs text-red-400 bg-red-500/10 border border-red-500/20 rounded-glass px-2.5 py-1.5">
