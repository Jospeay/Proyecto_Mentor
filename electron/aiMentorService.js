 ...
 *
 * Soporta: Google Gemini API (gemini-1.5-flash / gemini-2.0-flash)
 * ==============================================================================
 */
...
async function analyzeWorkload({ newTasks = [], existingTasks = [], subjects = [], geminiApiKey }) {
  ...
  if (geminiApiKey) {
    try {
      return await analyzeWithGemini({ newTasks, pendingTasks, subjects, geminiApiKey });
    } catch (err) {
      console.warn('[AI MENTOR] Gemini falló, usando análisis local:', err.message);
    }
  }
...
/**
 * Análisis real con la API de Google Gemini.
 */
async function analyzeWithGemini({ newTasks, pendingTasks, subjects, geminiApiKey }) {
  const url = `${GEMINI_API_BASE}/${DEFAULT_MODEL}:generateContent?key=${geminiApiKey}`;
  ...
  try {
    ...
    // Si Gemini no devolvió JSON válido, usar el texto directamente
    return {
      success: true,
      source: 'gemini',
      riskLevel: determineRiskFromText(textResponse),
      message: textResponse.slice(0, 500),
      suggestedPriority: [],
      calendarAdjustment: '',
      detailedAnalysis: textResponse,
    };
  }
}
